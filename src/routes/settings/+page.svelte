<script lang="ts">
    ...
    const saveDataframe  = async () => {
        ...
        await axios.post('/api/update-items', {updateSettings: 1, id, settings: dataFrame})
        .then(res => {
    ...
    const handleForm = (element:any) => {
        ...
        for(let key of keys){
            ...
            else if(element.target[key].type === 'checkbox')
                v = element.target[key].checked
            else
                v = element.target[key].value
            dataFrame[key] = v
        }
        saveDataframe()
    }
